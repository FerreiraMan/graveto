...
public class UpdateTransactionIT extends MoneyTrackerBaseIntegrationTest {
  ...
  void shouldUpdateTransaction() {
    ...
    final Category newCategory = categoryList.stream()
        .filter(c -> !c.equals(initialCategory))
        .filter(c -> !SystemCategory.isFallback(c.getName()))
    ...
    final Transaction transaction =
        TransactionTestFactory.createTransaction(account, initialCategory, TransactionType.INCOME,
    ...
    assertThat(updatedTransaction.getOccurredAt().truncatedTo(ChronoUnit.MILLIS)).isEqualTo(
