...
public class CreateTransferIT extends MoneyTrackerBaseIntegrationTest {
  ...
  void shouldCreateTransfer() {
    ...
    final Account sourceAccount =
        AccountTestFactory.createAccountWithOwner(userSid, "Santander", sourceAccountInitialBalance);
    final Account destinationAccount =
        AccountTestFactory.createAccountWithOwner(userSid, "BCP", destinationAccountInitialBalance);
    ...
    final Transaction outTx =
        transactions.stream().filter(t -> t.getType() == TransactionType.TRANSFER_OUT).findFirst().orElseThrow();
    final Transaction inTx =
        transactions.stream().filter(t -> t.getType() == TransactionType.TRANSFER_IN).findFirst().orElseThrow();
    ...
    assertThat(updatedSourceAccount.getBalance()).isEqualByComparingTo(
        sourceAccountInitialBalance.subtract(transferAmount));
    assertThat(updatedDestinationAccount.getBalance()).isEqualByComparingTo(
        destinationAccountInitialBalance.add(transferAmount));
  }
