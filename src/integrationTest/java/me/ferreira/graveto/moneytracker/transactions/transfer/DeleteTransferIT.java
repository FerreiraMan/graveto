...
public class DeleteTransferIT extends MoneyTrackerBaseIntegrationTest {
  ...
  void shouldDeleteTransferTransactions() {
    ...
    final Account outAccount =
        AccountTestFactory.createAccountWithOwner(userSid, "Santander", firstAccountInitialBalance);
    final Account inAccount =
        AccountTestFactory.createAccountWithOwner(userSid, "Santander", secondAccountInitialBalance);
    ...
    assertThat(updatedInAccount.getBalance()).isEqualByComparingTo(
        secondAccountInitialBalance.subtract(inTx.getAmount()));
  }
