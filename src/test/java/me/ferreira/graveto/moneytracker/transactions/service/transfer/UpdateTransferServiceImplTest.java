...
public class UpdateTransferServiceImplTest {
  ...
  void shouldUpdateTransferTransactionsAndAccountBalancesIfApplicable(
    ...
    final Category outCategory =
        CategoryUtils.createCategory("Transfer Out", userSid, null, true, TransactionType.TRANSFER_OUT);
    final Category inCategory =
        CategoryUtils.createCategory("Transfer In", userSid, null, true, TransactionType.TRANSFER_IN);
    final String persistedDescription = "Initial Transfer";
    final LocalDateTime persistedOccurredAt = LocalDateTime.now().minusDays(2);

    final Transaction txOut =
        createTransaction(outAccount, outCategory, persistedAmount, persistedDescription, TransactionType.TRANSFER_OUT,
            persistedOccurredAt);
    final Transaction txIn =
        createTransaction(inAccount, inCategory, persistedAmount, persistedDescription, TransactionType.TRANSFER_IN,
            persistedOccurredAt);
