...
public class UpdateRecurringTransferControllerTest {
  ...
  private static Stream<Arguments> invalidRequests() {
    return Stream.of(
        Arguments.of(new UpdateRecurringTransferRequestDto(
  ...
  void shouldReturnBadRequestForInvalidPayloads(
      final UpdateRecurringTransferRequestDto request,
      final String expectedErrorField) {

    final MvcTestResult result = mvc.patch()
        .uri("/recurring-transfers/" + UUID.randomUUID())
  ...
  @Test
  void shouldUpdateRecurringTransferSuccessfully() {
    // Arrange
    ...
    when(service.updateRecurringTransfer(commandCaptor.capture())).thenReturn(mockRt);
    ...
    final UpdateRecurringTransferCommand captured = commandCaptor.getValue();
  ...
  void shouldTrimDescriptionBeforePassingToService() {
    ...
    final RecurringTransfer mockRt = buildMockRecurringTransfer(UUID.randomUUID(), accountSid, UUID.randomUUID());
  ...
  private RecurringTransfer buildMockRecurringTransfer(final UUID rtSid, final UUID accountSid,
                                                       final UUID userSid) {
    final Account sourceAccount = new Account();
    sourceAccount.setSid(accountSid);
    ...
    destinationAccount.setSid(accountSid);
